<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Money[]|\Cake\Collection\CollectionInterface $money
 */
?>
<div class="money index content">
    <?= $this->Html->link(__('New Money'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Money') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('name') ?></th>
                    <th><?= $this->Paginator->sort('value') ?></th>
                    <th><?= $this->Paginator->sort('urlData') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($money as $money): ?>
                <tr>
                    <td><?= $this->Number->format($money->id) ?></td>
                    <td><?= h($money->name) ?></td>
                    <td><?= h($money->value) ?></td>
                    <td><?= h($money->urlData) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $money->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $money->id]) ?>
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $money->id], ['confirm' => __('Are you sure you want to delete # {0}?', $money->id)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>
